...
class GameMech:
    ...
    def create_world(self):
        ...
        '''
        for i in walls:
            self.add_obstacle("wall", i[0], i[1])

        for i in dots:
            self.add_obstacle("dot", i[0], i[1])

    def is_dot(self, x, y, name):
        if name == "pacman":
            for e in self._world[(x, y)]:
                if e[1] == "dot":
                    world_pos = self._world[(x, y)]
                    world_pos.remove(world_pos[0])
                    return True
                return False

    def count_dots(self):
        c = 0
        for l in self._world.values():
            for i in l:
                if i[1] == "dot":
                    c += 1
        self._nr_dots = c
    ...
    def execute(self,move: int, type:str, nr_player:int ) -> tuple:
        ...
        if type == "player":
            ...
            if (next_tick - tick) > TIME_STEP:
                ...
                self.is_dot(new_pos_x, new_pos_y, self.players[nr_player][0])
