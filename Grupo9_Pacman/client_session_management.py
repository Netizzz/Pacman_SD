...
class ClientSession(Thread):
    ...
    def process_close(self, s_c):
        close = self.gm.count_dots()
        s_c.send(pickle.dumps(close))

    def process_death(self, s_c):
        close = self.gm.death()
        s_c.send(pickle.dumps(close))
    ...
    def dispatch_request(self, socket_client) -> bool:
        ...
        elif data_str == constants.CLOSE:
            self.process_close(socket_client)
        elif data_str == constants.DEATH:
            self.process_death(socket_client)
