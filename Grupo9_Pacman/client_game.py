...
import random
import client_stub
import player_client, wall_client, player_other
...
class GameUI(object):
    ...
    def set_walls(self, wall_size:int):
        ...
        for nr in range(nr_obstacles):
            if self.wl[str(nr)][0] == "wall":
                ...
                wall = wall_client.Wall(w_x, w_y, self.grid_size, "wall", self.walls)
                self.walls.add(wall)
            if self.wl[str(nr)][0] == "dot":
                w_x, w_y = self.wl[str(nr)][1][0], self.wl[str(nr)][1][1]
                dot = wall_client.Wall(w_x, w_y, self.grid_size, "dot", self.walls)
                self.dots.add(dot)
    ...
    def run(self):
        ...
        while end == False:
            ...
            if self.stub.close_game() or self.stub.game_death():
                end = True
